...
# salvager: an open-source agent that finds the exact second-hand homelab parts you want — even when they're hidden inside someone else's listing.

## Catches the listings you'd buy if you saw them in time.

**Valencia, [Launch Date]** — Good second-hand listings on Wallapop and eBay.es vanish in about four hours, which means a homelabber hunting for a specific HDD or RAM kit either checks every twenty minutes or accepts that the best deals will always be gone. **salvager**, released today as an open-source project, watches both marketplaces continuously and sends a Telegram alert the moment a real match against a wishlist appears — including the listings where the part you want is hiding inside something else.
...
Setup is a single repository clone and a configuration file. A homelabber installs salvager on a box they already run — a NAS, a homelab server, a small VPS — and creates a Telegram bot in a couple of minutes.
...
salvager is released under the **MIT** license at **github.com/ifuensan/salvager**. Installation is documented in the README: clone the repository, copy `.env.example`, fill in a Telegram bot token and the marketplace credentials, write a YAML wishlist (the repository ships with example entries for common HDD and RAM models), and start the agent with a single docker-compose command. A first alert typically arrives within hours of the first wishlist entry being added.
...
Contributions are welcome: example wishlist entries for common homelab parts, prompt improvements for the listing-evaluation step, and bug reports against Wallapop's anti-bot evolution all live as standard GitHub pull requests and issues. The roadmap, including a future research direction on adapting salvager to non-Spanish marketplaces, is published at **github.com/ifuensan/salvager/blob/main/ROADMAP.md**.
...
If a homelabber's needs are covered by keyword alerts on saved searches, the existing tools are fine. The case for salvager is wishlist precision, container detection, two-marketplace coverage, and not losing a deal because you were in a meeting.
...
A: Saved searches work great for "ping me when 'WD Red' shows up under €X" and a homelabber whose hunt fits that pattern should keep using them. They don't, however, do three things salvager does:
...
A: The ToS forbids automated access. The realistic risk is platform-side account termination (not legal action — Spanish courts have generally permitted scraping of public data). salvager mitigates by polling at human-volume rates, running listing fetches through stealth Chromium when the unofficial API is blocked, and never doing mass scraping or unattended purchases. Recommended: run with a Wallapop account you'd be willing to lose, not your primary trading account.
...
Honest answer: this can become unmaintained. The trade-off is that "self-hosted, MIT, no SaaS lock-in" is the cleanest one-person-project risk profile you can get. If long-term continuity matters more than wishlist precision and container detection, salvager isn't the right tool.
...
Honest answer: expect a couple of break/patch cycles a year, expect Phase 2 buy flows to be the most fragile part (UI selectors change), and expect eBay.es to be more stable than Wallapop. If you can't tolerate any downtime, salvager isn't the right fit.
...
A: Honest answer: we don't have real-world numbers yet — salvager is launching, not running with users for a year. What we can say:
...
A: The user eats the loss — salvager offers no insurance. What stands between the user and that loss:
...
- **What we don't do:** publicly fight, name Wallapop in litigation, or bait them. The README positions salvager as a personal monitoring tool, not a "Wallapop scraper." Wording matters.
...
RGPD posture: the log lives entirely on the user's machine. No remote logging, no telemetry. Sellers' publicly posted data is processed only for the duration of the transaction; no profiling, no third-party sharing. The user is the data controller for their own log; salvager is the processor.
...
- **Personal-use trigger:** I stop using salvager for 3+ consecutive months for my own homelab.
...
- **Critical bug:** pin to last known-good version, raise upstream issue, ship a release with the pin. Steady-state salvager doesn't depend on cutting-edge Hermes.
...
salvager survived the gauntlet. Honest assessment:
...
- Q3 (RGPD/consumer law evidence): three artifacts persisted locally per Phase 2 purchase (alert snapshot, tap event, marketplace transaction). Append-only SQLite log. User = data controller; salvager = processor.
...
- **Q4 (one-person OSS sustainability):** picked option (A) — distinguishes "developer abandons" from "marketplace breaks compatibility." Honest closing: "if long-term continuity matters more than wishlist precision, salvager isn't the right tool."
...
- Open kickoff-doc questions still unresolved: project name (assumed final = "salvager"), session persistence strategy, browser-failure fallback, rate-limiting per execution, container "worth it" decision criterion, web-backend choice (Firecrawl vs SearXNG), API vs Google index, agentskills.io publication.
