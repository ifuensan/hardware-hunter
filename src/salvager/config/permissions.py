...
from salvager.config.env import ENV_AUTH_EXIT_CODE
...
class CredentialMissingError(CredentialError):
    ...
    The operator has not yet run the matching ``salvager login``
...
def _missing_hint(path: Path) -> str:
    ...
    name = path.name
    if name == ".env":
        return "run salvager init, then fill in .env"
    if "wallapop" in name:
        return "run salvager login wallapop"
    if "oauth" in name or "ebay" in name:
        return "run salvager login ebay"
    return "run the matching salvager login subcommand"
