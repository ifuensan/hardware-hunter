...
operator re-runs ``hardware-hunter login ebay``.
...
from hardware_hunter.adapters.ebay_api.schema import (
    EbayApiItem,
...
class EbayApiFetcher(PageFetcher):
    ...
    async def _refresh_tokens(self) -> OAuthTokens:
        ...
        if response.status_code == 401:
            ...
            raise EbayAuthFailed("eBay refresh token rejected — run `hardware-hunter login ebay`")
