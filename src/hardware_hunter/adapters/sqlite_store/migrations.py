...
Discovers ``NNNN_*.sql`` files in ``hardware_hunter.migrations``,
...
class MigrationRunner:
    ...
    def __init__(self, package: str = "hardware_hunter.migrations") -> None:
...
def db_path_under(data_dir: str | Path) -> Path:
    """Return the canonical ``hardware_hunter.db`` path under ``data_dir``."""
    return Path(data_dir) / "hardware_hunter.db"
